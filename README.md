...
Webhook de **deploy continuo** del ecosistema CloserClick. Un servicio chico (Node
...
```bash
git clone https://github.com/closerclick/cc-deploy-listener.git ~/cc-deploy-listener
...
Por cada repo que despliega el host (con la cuenta `closerclick`):

```bash
gh api -X POST repos/closerclick/simple-websocket-proxy/hooks \
  -f name=web -F active=true -f 'events[]=push' \
  -f config[url]=https://proxy.closer.click/hooks/deploy \
